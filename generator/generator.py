...
\"\"\"
from CasaTunes.objects.objects.base import CasaBase

{inherit}
"""

CLASS = """
class {classname}(CasaBase):
{properties}
"""
...
def generateclass(name, data, primary=False):
    ...
    INHERIT.append(
        CLASS.format(classname=f"CasaTunes{classname}", properties="".join(properties))
    )

    objectfilename = f"pycasatunes/objects/{'/'.join([x.lower() for x in re.findall('[A-Z][a-z]*', classname)])}.py"
    ...
    with open(
        ...
        objfile.write(
            OUT.format(
                classname=f"CasaTunes{classname}",
    ...
    with open(
        ...
        fixturefile.write(
            TEST.format(
                ...
                importclass=f"CasaTunes{classname}",
